use crate::jsonrpc::{
    error::RpcError,
    web3_types::{
        BlockId, FatTransactionOrHash, Web3Block, Web3CallRequest, Web3FeeHistory, Web3Filter,
...
use rt_evm_model::{
    ...
    types::{
        ...
        SignedTransaction, TxResp, UnverifiedTransaction, H160, H256, H64, MAX_BLOCK_GAS_LIMIT,
        MIN_GAS_PRICE, U256,
    },
};
use ruc::*;
...
impl<Adapter: APIAdapter> Web3RpcImpl<Adapter> {
    ...
    fn call_evm(&self, req: Web3CallRequest, data: Bytes, number: Option<u64>) -> Result<TxResp> {
...
impl<Adapter: APIAdapter + 'static> RTEvmWeb3RpcServer for Web3RpcImpl<Adapter> {
    ...
    async fn get_tx_count(&self, address: H160, number: Option<BlockId>) -> RpcResult<U256> {
    ...
    async fn get_balance(&self, _address: H160, _number: Option<BlockId>) -> RpcResult<U256> {
        Ok(0.into())
    }
    ...
    async fn estimate_gas(&self, req: Web3CallRequest, number: Option<BlockId>) -> RpcResult<U256> {
        ...
        if resp.exit_reason.is_succeed() {
            ...
            let mut mid = (U256::from(resp.gas_used) * U256::from(3)).min((highest + lowest) / 2);
