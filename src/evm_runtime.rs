...
pub use model::types::transaction::SignedTransaction as SignedTx;
...
impl EvmRuntime {
    ...
    pub fn create(chain_id: u64, token_distributions: &[TokenDistributon]) -> Result<Self> {
    ...
    pub fn restore() -> Result<Option<Self>> {
        ...
        match (chain_id, trie_db, storage) {
            ...
            (Err(a), Err(b), Err(c)) => match (a.kind(), b.kind(), c.kind()) {
                (ErrorKind::NotFound, ErrorKind::NotFound, ErrorKind::NotFound) => Ok(None),
    ...
    pub fn generate_blockproducer(&self, proposer: H160, timestamp: u64) -> Result<BlockMgmt> {
        BlockMgmt::new(
            proposer,
            timestamp,
    ...
    pub fn check_signed_tx(&self, tx: &SignedTx) -> Result<()> {
        self.mempool.tx_pre_check(tx, true)
    }
